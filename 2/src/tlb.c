...
void
tlb_add_entry (struct tlb *tlb, uint16_t page_number, int16_t frame_number)
{
  // Complétez cette fonction.
  int i = findFreeEntry(tlb);
    ...
    if(i!= -1){
        ...
        replaceWithLFU(tlb,page_number, frame_number);
        
    }
}

void
replaceWithLFU(struct tlb *tlb, uint16_t page_number, int16_t frame_number){
    int min = tlb->entries[0].use;
    int index = 0;
    for(int i=0; i<TLB_NUM_ENTRIES; i++){
        if(tlb->entries[i].use < min){
            index = i;
            min = tlb->entries[i].use;
        }

    }
    tlb->entries[index].page_number = page_number;
    tlb->entries[index].frame_number = frame_number;
    tlb->entries[index].use = 1;
}
